...
## t-statistics of beta2 by 2SQR(1), including xt-2
n      = ceiling(748 * 5/4)  # the sample size = 700, discard the first quarter
ns     = ceiling(n/4)
    ...
    for (k in 1:length(a)) {
        for (p in 1:length(b2)) {
            for (i in 1:nsim) {
                ...
                # scale = sigma * nu[3]/(nu[3] - 2) 
                # innov = rmvt(n, sigma = scale, df = nu[3]) # t errors
                e      = innov[, 2]
                u      = innov[, 1]
                x      = rep(0, n)
                x[1]   = e[1]
...
## estimate the denstiy of the statistics
dd2 = density(stat2)
par(mgp = c(1.5, 0.5, 0))
plot(dd2, main = "t-statistics", ylab = "Kernel Density", xlab = "")
lines(seq(-4, 4, 0.2), dnorm(seq(-4, 4, 0.2), 0, 1), col = "blue")

## self normalized statistics of beta2 by 2SQR(2), not including xt-2
n      = ceiling(748 * 5/4)  # the sample size = 700, discard the first quarter
ns     = ceiling(n/4)
nsim   = 1000  # the times of repeats in simulation
TAU    = 0.1
    ...
    for (k in 1:length(a)) {
        for (p in 1:length(b2)) {
            for (i in 1:nsim) {
                ...
                beta2[i] = BETA3[[1]]
                stat2[i] = f.u_tau * sqrt(x[(ns + 2):n] %*% x[(ns + 2):n] - sum(x[(ns + 
                  2):n])^2/(n - ns - 1)) * (beta2[i])/sqrt(TAU[t] * (1 - TAU[t]))
                if (sign(pnorm(abs(stat2[i])) - 0.975) == 1) {
                  reject[[t]][k, p] = reject[[t]][k, p] + 1
                }
            }
...
dd2 = density(stat2)
par(mgp = c(1.5, 0.5, 0))
plot(dd2, main = "self normalized statistics", ylab = "Kernel Density", xlab = "")
lines(seq(-4, 4, 0.2), dnorm(seq(-4, 4, 0.2), 0, 1), col = "blue")
